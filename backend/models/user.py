import datetime
from backend.db import get_db_connection, get_engine_name
...
class UserModel:
    ...
    def get_profile(user_id=1):
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT COUNT(*) as completed_count FROM tasks WHERE user_id = %s AND status = 'complete';", (user_id,))
            completed_row = cur.fetchone()
            total_completed = completed_row.get("completed_count", 0) if completed_row else 0

            # Count missed tasks
            cur.execute("SELECT COUNT(*) as missed_count FROM tasks WHERE user_id = %s AND (status = 'missed' OR (status = 'pending' AND deadline IS NOT NULL AND deadline < CURRENT_TIMESTAMP));", (user_id,))
    ...
    def modify_xp(user_id, amount, reason=""):
        conn = get_db_connection()
        with conn.cursor() as cur:
            cur.execute("SELECT xp, level, title FROM users WHERE id = %s;", (user_id,))
            user = cur.fetchone()
            if not user:
                conn.close()
                return None
        ...
        return {
            ...
            "change": amount,
            "level": new_level,
            "leveled_up": new_level > (user.get("level") or 1),
            "leveled_down": new_level < (user.get("level") or 1),
            "profile": profile
        }
        ...
        with conn.cursor() as cur:
            ...
            if last_active_raw:
                ...
                if delta == 0:
                    ...
                    streak += 1
                    streak_gained = True
                    cur.execute("UPDATE users SET streak = %s, last_active = %s WHERE id = %s;", (streak, today.isoformat(), user_id))
                else:
                    # Skipped 2 or more days
                    streak_lost = True
                ...
                streak = 1
                streak_gained = True
                cur.execute("UPDATE users SET streak = 1, last_active = %s WHERE id = %s;", (today.isoformat(), user_id))
    ...
    def reset_progress(user_id=1):
        ...
        with conn.cursor() as cur:
            # 1. Reset user xp, streak, title, last_active
            cur.execute("""
                UPDATE users 
                SET xp = 0, streak = 0, 
